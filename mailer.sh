...
if [ ! -z $(grep "DATABASE_ENABLE=TRUE" config.mailer) ]
then
...
if [ ! -z $(grep "DATABASE_ENABLE" $BUFF/config.mailer) ]
then
...
if [ $( cat /proc/uptime | cut -d " " -f 1 | cut -d "." -f 1) -le 600 ]
then 
MOD="REBOOT"
echo "$(date): INFO: System has been rebooted recently" >> $LOG_FILE
echo "INFO: System has been rebooted recently" >> $MAIL_BODY
fi
#================================ IP form check ================================
#192.168.  1.  2
#  5. 10.122.223
...
then
	...
	then
		echo "$(date): WARING: Latency value reached! Latency value is $PING_AVG" >> $LOG_FILE
		echo "WARNING: Latency value reached! Latency value is $PING_AVG" >> $MAIL_BODY
	...
	echo "$(date): Info: Ping monitor disable" >> $LOG_FILE
fi
#==========================================================================
if [ -s "$MAIL_BODY" ]
then
...
echo "$(date): INFO: Database operations has be differed" >> $LOG_FILE
fi
fi
